...
NV21Image* crop_nv21(const NV21Image* src, int left, int top, int crop_w, int crop_h) {
    // 检查裁剪范围是否合法
    if (left < 0 || top < 0 ||
        left + crop_w > src->width ||
        top + crop_h > src->height) {
        printf("裁剪范围超出源图像范围！\n");
        return NULL;
        }
    ...
    for (int row = 0; row < crop_h; row++) {
        memcpy(cropped->y + row * crop_w,
               src->y + (top + row) * src->width + left,
               crop_w);
    }

    // 裁剪 VU 平面
    for (int row = 0; row < crop_h / 2; row++) {
        memcpy(cropped->vu + row * crop_w,
               src->vu + ((top / 2) + row) * src->width + left,
               crop_w);
    }
