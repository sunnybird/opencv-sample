...
void nv21_affine_transform(
    const uint8_t *src_nv21, int src_width, int src_height,
    uint8_t *dst_nv21, int dst_width, int dst_height,
    const Mat &affine_mat) {
    ...
    // 仿射变换 Y
    Mat dst_y;
    warpAffine(src_y, dst_y, affine_mat, Size(dst_width, dst_height), INTER_LINEAR, BORDER_CONSTANT, Scalar(0));
    ...
    Mat dst_vu_up;
    warpAffine(src_vu_up, dst_vu_up, affine_mat, Size(dst_width, dst_height), INTER_LINEAR, BORDER_CONSTANT,
               Scalar(128, 128));
...
inline
Mat getRotationMatrix2D_1(Point2f center, double angle, double scale) {
...
int main() {
    ...
    const int dst_width = 320, dst_height = 240;
    ...
    // === 加载 NV21 文件 ===
    ifstream fin("../input_640x480.nv21", ios::binary);
    if (!fin) {
    ...
    // === 执行仿射变换 ===
    nv21_affine_transform(src_nv21.data(), src_width, src_height,
                          dst_nv21.data(), dst_width, dst_height,
                          affine_mat);

    // === 写出结果 ===
    ofstream fout("../output_320x240.nv21", ios::binary);
    fout.write((char *) dst_nv21.data(), dst_size);
    fout.close();

    cout << "✅ 仿射变换完成，输出：output_320x240.nv21" << endl;
    return 0;
}

// ffplay -f rawvideo -pixel_format nv21 -video_size 320x240 output_320x240.nv21

// ffmpeg -f rawvideo -pixel_format nv21 -video_size 320x240 -i output_320x240.nv21 -frames:v 1 output_320x240.png
